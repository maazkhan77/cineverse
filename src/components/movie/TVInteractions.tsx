...
export function TVInteractions({ 
  tvId, 
  tvName, 
  posterPath, 
  voteAverage,
  ...
  const isInWatchlist = useQuery(api.watchlist.isInWatchlist, { tmdbId: tvId });
  const userRating = useQuery(api.ratings.getUserRating, { tmdbId: tvId, mediaType: "tv" });
  const addToWatchlist = useMutation(api.watchlist.add);
  const removeFromWatchlist = useMutation(api.watchlist.remove);

  const handleWatchlistToggle = async () => {
    if (!isAuthenticated) {
      router.push(`/login?redirect=/tv/${tvId}`);
      return;
    }
    ...
    try {
      if (isInWatchlist) {
        await removeFromWatchlist({ tmdbId: tvId });
        ...
        toast.success(`${tvName} added to watchlist!`);
      }
    } catch (error) {
  ...
  const handleShare = async () => {
    const url = `${window.location.origin}/tv/${tvId}`;
    if (navigator.share) {
      try {
        await navigator.share({ title: tvName, url });
      } catch (err) {
